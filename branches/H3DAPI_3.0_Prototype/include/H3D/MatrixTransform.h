...
namespace H3D {
  ...
  public:
	typedef H3D::SFMatrix4f SFMatrix4f;
		...
		/// The IsDirty field is updated each scenegraph loop, since
		/// Scene::time is routed to it to see if this class was 
		/// set to dirty during or since the last loop. 
			class DirtyFlag : public Field {
			public:
				/// Constructor.
				DirtyFlag() {
				}
	...
	///	This is a dirty flag that we can use to flag if this 
	///	object and all its' children should be updated or not.
	///	If isDirty is true then we traverse through this object and all 
	///	its' children and update all values.
	/// 
	/// <b>Access type:</b> inputOutput \n
	/// <b>Default value:</b> True \n
	auto_ptr< DirtyFlag > isDirty;

	
	///	This is a global dirty flag that is shared between all transform objects (and the derived children).
	/// The point of it is that if no object at all was changed, we don't need to traverse or update anything.
	/// 
	/// <b>Access type:</b> inputOutput \n
	/// <b>Default value:</b> True \n
	static auto_ptr< DirtyFlag > globalTraverseFlag;
