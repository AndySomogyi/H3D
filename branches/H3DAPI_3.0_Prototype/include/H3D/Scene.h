...
namespace H3D 
{
	...
	public:
		...
		*/
		...
		/// Field value associated with a name. These settings can be used in
		...
		/// \param name The name of the parameter. If "" the name of the
		/// field will be used.
		/// \param section The type if parameter it is, e.g. graphics.
		...
		static void findNodes ( 
			...
			Node* _parent= NULL ) 
		{
			if (_typeNames == NULL || find( _typeNames->begin(), _typeNames->end(), _node.getTypeName()) != _typeNames->end()) 
			{
			...
			for( H3DNodeDatabase::FieldDBConstIterator i = db->fieldDBBegin(); db->fieldDBEnd() != i; ++i) 
			{
