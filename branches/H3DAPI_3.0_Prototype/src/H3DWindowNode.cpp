...
void H3DWindowNode::renderChild( X3DChildNode *c,
	bool render_to_fbo ) 
{
	H3DDisplayListObject *dlo = dynamic_cast< H3DDisplayListObject * >( c );
...
void H3DWindowNode::render( X3DChildNode *child_to_render)
{
	...
	if( !isSinglePass ) 
	{ 
		// need to render twice for stereo
		...
		{ // set viewport for left eye, when the viewport not covering the whole window
			if( viewports_size[4] != 0 || viewports_size[5] != 0 ||
				viewports_size[6] != width->getValue() || viewports_size[7] != height->getValue() ) 
				{
					...
					fbo_current_x = static_cast<H3DUtil::H3DInt32>(viewports_size[4]);
					fbo_current_y = static_cast<H3DUtil::H3DInt32>(viewports_size[5]);
				}
		}
		...
		if( background ) {
			glPushMatrix();
			glRotatef( (H3DFloat) -(180.0f/Constants::pi)*vp_orientation.angle, 
				vp_orientation.axis.x, 
				vp_orientation.axis.y,
				vp_orientation.axis.z );
			glRotatef( (H3DFloat) (180.0f/Constants::pi)*vp_inv_rot.angle, 
		...
		if( stereo_mode == RenderMode::QUAD_BUFFERED_STEREO ) {
			...
			if( viewports_size[8] != 0 || viewports_size[9] != 0 ||
				viewports_size[10] != width->getValue() || viewports_size[11] != height->getValue() ) {
					...
					fbo_current_x = static_cast<H3DUtil::H3DInt32>(viewports_size[8]);
					fbo_current_y = static_cast<H3DUtil::H3DInt32>(viewports_size[9]);
			}
		}
