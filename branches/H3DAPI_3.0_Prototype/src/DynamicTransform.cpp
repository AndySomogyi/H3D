...
MatrixTransform( _addChildren, _removeChildren, _children,
	...
	motion          ( _motion           )   {
		...
		angularVelocity->routeNoEvent(isDirty);
		orientation->routeNoEvent(isDirty);
		angularMomentum->routeNoEvent(isDirty);
...
void DynamicTransform::SFMotion::update() {
	...
	//Values have changed so flag our parent for traverseSG
