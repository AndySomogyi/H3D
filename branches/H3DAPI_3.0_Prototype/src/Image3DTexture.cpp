	...
	const NodeVector &image_loaders ) 
{
	...
	//Do a lookup on this specific URL
	...
	if( image_loaders.size() ) { 
		for( vector<string>::const_iterator i = urls.begin(); i != urls.end(); ++i ) {
			for( NodeVector::const_iterator il = image_loaders.begin(); il != image_loaders.end(); ++il ) {
			...
			//If we've reached this point, none of the image loaders specified have worked. Hence we try other ways of extracting the image.
			...
			if ( url_contents != "" ) {
				istringstream tmp_istream( url_contents );
				auto_ptr< H3DImageLoaderNode > il( H3DImageLoaderNode::getSupportedFileReader( tmp_istream ) );
				if( il.get() ) {
					texture->setURLUsed( *i );
					image = il->loadImage( tmp_istream );
					//Save the image for future lookups to save on texture load time.
					tm.insertNewImageURLPairing(*i, image);
					return image;
				}
	...
	//If we've reached this point, nothing has worked. Bad.
