...
void IndexedTriangleSet::render() 
{
	...
	bool tex_coord_gen = !tex_coord_node || (tex_coord_node && tex_coord_node->supportsTexGen());

	bool tex_coords_per_vertex = tex_coord_node && tex_coord_node->supportsExplicitTexCoords();
	...
	if( coordinate_node && !indices.empty() ) 
	{
		...
		if( tex_coords_per_vertex && coordinate_node->nrAvailableCoords() > tex_coord_node->nrAvailableTexCoords() ) 
		{
			...
			if( tex_coord_node->nrAvailableTexCoords() != -1 ) 
			{
				...
				s << "Must contain at least as many elements as coord (" 
					...
					<< getName() << "\" node. ";
				throw NotEnoughTextureCoordinates( tex_coord_node->nrAvailableTexCoords(), s.str(), H3D_FULL_LOCATION );
			}
		}
		...
		if( normalPerVertex->getValue() ) 
		{
			...
			prefer_vertex_buffer_object = true;
...
void IndexedTriangleSet::traverseSG( TraverseInfo &ti ) 
{
	...
	if(!render_tangents && !ti.getUserData( "shaderRequiresTangents", 
	...
	renderData.renderState = ti.GetCurrentRenderstate();
	...
	if(!vboFieldsUpToDate->isUpToDate() || (!attributeLayoutCached))
	{
		...
		if(coordinate_node)
		{
			...
			BindlessPtrNV coordinateAttribute;
			...
			attribDescription.size = coordinate_node->getElementSize();
			attribDescription.type = VERTEXATTRIBUTE::VERTEX;
		...
		if(tex_coord_node)
		{
			...
			BindlessPtrNV textureCoordinateAttribute;
			...
			attribDescription.index = attributeIndexCount;
			attribDescription.size = tex_coord_node->getElementSize();
			attribDescription.type = VERTEXATTRIBUTE::TEXCOORD;
		...
		if(fog_coord_node)
		{
			...
			BindlessPtrNV fogCoordinateAttribute;
			...
			attribDescription.index = attributeIndexCount;
			attribDescription.size = fog_coord_node->getElementSize();
			attribDescription.type = VERTEXATTRIBUTE::FOGCOORD;
		...
		if(color_node)
		{
			...
			attribDescription.index = attributeIndexCount;
			attribDescription.size = color_node->getElementSize();
			attribDescription.type = VERTEXATTRIBUTE::COLOR;
		...
		if(normal_node)
		{
			...
			attribDescription.index = attributeIndexCount;
			attribDescription.size = normal_node->getElementSize();
			attribDescription.type = VERTEXATTRIBUTE::NORMAL;
...
void IndexedTriangleSet::AutoNormal::update() 
{
	...
	X3DCoordinateNode *coord = 	static_cast< X3DCoordinateNode * >( static_cast< SFCoordinateNode * >( routes_in[1] )->getValue() );
...
void IndexedTriangleSet::AutoTangent::update() 
{
	...
	X3DCoordinateNode* coord = 
		static_cast< X3DCoordinateNode * >( static_cast< SFCoordinateNode * >( routes_in[1] )->getValue() );

	const vector<int>& index = static_cast< MFInt32 * >( routes_in[2] )->getValue();

	X3DTextureCoordinateNode *tex_coord = 
		static_cast< X3DTextureCoordinateNode * >( static_cast< SFTextureCoordinateNode * >( routes_in[3] )->getValue() );
	...
	bool ccw ) 
{
	...
	if( coord ) 
	{
		...
		for(vector<Vec3f>::iterator i = normals.begin(); i != normals.end(); ++i) 
		{
			(*i).normalizeSafe();
		}
...
Vec3f IndexedTriangleSet::AutoTangent::getTexCoord( X3DCoordinateNode *coord,
	X3DTextureCoordinateNode *tex_coord, int index ) 
{
	if( tex_coord ) 
	{
...
void IndexedTriangleSet::AutoTangent::calculateTangent( const Vec3f &a, const Vec3f &b, const Vec3f &c,
	const Vec3f &ta, const Vec3f &tb, const Vec3f &tc, Vec3f &tangent, Vec3f &binormal ) 
{
	const Vec3f& v1 = a;
	const Vec3f& v2 = b;
	const Vec3f& v3 = c;
