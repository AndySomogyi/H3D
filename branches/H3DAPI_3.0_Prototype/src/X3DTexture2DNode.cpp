...
void X3DTexture2DNode::glTexImage( Image *i, GLenum texture_target,
	bool scale_to_power_of_two ) {
		...
		if( scale_to_power_of_two && !GLEW_ARB_texture_non_power_of_two ) {
			...
			if( needs_scaling ) {	
				// When calling specifically this PixelImage constructor, it
				// fetches height and width from the Image* and compares it with
				// new_height and new_width, if they're different it does a basic 
				// linear sub/supersample to make sure it scales properly
				//Also, we just send in a 1 because this isn't a 3Dtex, hence depth==1
				pi.reset(new PixelImage(i, new_width, new_height, 1));
				width = new_width;
				height = new_height;
		...
		TextureManager::getInstance().buildTexture(texture_handle, i);
...
void X3DTexture2DNode::render()     {
	...
	if( displayList->hasCausedEvent( image ) || texture_target != target ) {
		if( !image->imageChanged() || texture_handle.ogl_texture_id == 0 || texture_target != target ) {
			// the image has changed so remove the old texture and install the new
			
			//TODO: We need to call texture manager to handle this.
			glDeleteTextures( 1, &texture_handle.ogl_texture_id );
			texture_handle.ogl_texture_id = 0;
			texture_target = target;

			if( i ) {
				texture_handle = renderImage( i, 
					texture_target, 
					scaleToPowerOfTwo->getValue(), textureProperties->getValue());
			}
		} else {
		...
		if ( texture_handle.ogl_texture_id ) {
...
void X3DTexture2DNode::renderSubImage( Image *image, GLenum texture_target, 
	int x_offset, int y_offset, int width, int height ) {
		...
		unsigned char *modified_data = new unsigned char[ width * height * bytes_per_pixel ]; 
