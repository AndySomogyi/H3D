...
X3DGeometryNode( _metadata, _bound, _displayList, _isTouched,
	...
	dirtyFlag(new Field)
 {
		...
		closureType->route( dirtyFlag );
		solid->route( dirtyFlag );
		
		renderable.renderMode = GL_TRIANGLE_FAN;
...
void ArcClose2D::render() {
	...
	//H3DFloat x, y;
	//H3DFloat nr_segments = nrTriangles();
...
void ArcClose2D::traverseSG( TraverseInfo &ti ) {
	...
	if(!dirtyFlag->isUpToDate()) {
		...
		if( solid->getValue()) {
			ti.getCurrentRenderstate().setFaceCullingEnabled(GL_TRUE);
			ti.getCurrentRenderstate().setCullFace(GL_BACK);
		} else {
			ti.getCurrentRenderstate().setFaceCullingEnabled(GL_FALSE);
		}
...
void H3D::ArcClose2D::buildVBO(TraverseInfo &ti)
{
	//We're supposed to fetch color from some form of material in ti, but that's not been implemented yet so it'll be white.
	Vec3f color(1.0f, 1.0f, 1.0f);

	//Normal doesn't change, so we write the same value to every attribute
	...
	H3DFloat theta, angle_increment;
	H3DFloat x, y;
	H3DFloat nr_segments = static_cast<float>(nrTriangles());
	...
	texture_coordinates.push_back(Vec2f( start_point_tc.x, start_point_tc.y));
	positions.push_back(Vec2f(start_point.x, start_point.y));

	for ( theta = start_angle; theta < end_angle; theta += angle_increment ) {
			x = r * H3DCos(theta);
			y = r * H3DSin(theta);
			texture_coordinates.push_back(Vec2f(x / (r*2) + 0.5f, y / (r*2) + 0.5f));
	...
	std::vector<Vec3f> normals(positions.size(), normal);
	...
	renderable.VBOs[TEXCOORD].buildBufferObjectSingleAttribute(GL_FLOAT, &texture_coordinates[0], texture_coordinates.size()*sizeof(Vec2f), texture_coordinates.size(), GL_FALSE, TEXCOORD);
	...
	renderable.VBOs[NORMAL].buildBufferObjectSingleAttribute(GL_FLOAT, &normals[0], normals.size()*sizeof(Vec3f), normals.size(), GL_TRUE, NORMAL);
}
