...
auto_ptr<MatrixTransform::DirtyFlag> MatrixTransform::globalTraverseFlag(new DirtyFlag());
	...
	transformedBound( _transformedBound )
{
	...
	/* Experimental stuff */
	isDirty->setName("MatrixTransform::isDirty");
	isDirty->upToDate(); //Always set to dirty==true initially to force full SG traversal
	//isDirty->routeNoEvent(globalTraverseFlag);
}
...
void MatrixTransform::traverseSG( TraverseInfo &ti ) {
#ifdef HAVE_PROFILER
	H3DTIMER_BEGIN("MatrixTransform::traverseSG in node: " + this->getName())
#endif
		//if(globalTraverseFlag->getValue()) {
		//Crude optimization. We only set AccumForward and AccumInverse if the values have changed (dirty flag == true) of this object or any parent.
		bool dirty = (!isDirty->isUpToDate()) | ti.transformParentDirty();

	//So if we are dirty, set the ti dirty flag to true to make sure that all children update their values.
	if(dirty) {
		ti.setDirtyTransformFlag();
	}

	ti.pushMatrices( matrix->getValue(), matrix->getValue().inverse() );
	...
	//}
#ifdef HAVE_PROFILER
	H3DTIMER_END("MatrixTransform::traverseSG in node: " + this->getName())
#endif
}
