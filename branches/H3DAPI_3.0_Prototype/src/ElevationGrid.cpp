...
void ElevationGrid::AutoNormal::update() {
	...
	if( normals_per_vertex && crease_angle > 0 ) {
		if( crease_angle < Constants::pi ) {
			value = generateNormalsPerVertex( x_dim, z_dim, x_spacing, z_spacing, height, ccw, crease_angle );
		} else {
			value = generateNormalsPerVertex( x_dim, z_dim, x_spacing, z_spacing, height, ccw );
		}
	}
}
...
void ElevationGrid::render() {
	...
	TextureCoordinateGenerator *tex_coord_gen = dynamic_cast< TextureCoordinateGenerator * >( tex_coord_node );
...
void ElevationGrid::stopTexGen( TextureCoordinateGenerator *tex_coord_gen ) {
	MultiTexture *mt = 
		dynamic_cast< MultiTexture * >( X3DTextureNode::getActiveTexture() );
	if( !tex_coord_gen ) {
		if( mt ) {
...
X3DNormalNode *ElevationGrid::AutoNormal::generateNormalsPerVertex(  
	H3DInt32 x_dim, H3DInt32 z_dim, H3DFloat x_spacing, H3DFloat z_spacing,
	const vector< H3DFloat > &height, bool ccw ) {

		Normal *normal = new Normal;

		if( x_dim > 1 && z_dim > 1 ) {
			vector< Vec3f > normals( x_dim * z_dim, Vec3f( 0, 0, 0 ) );
			AutoRef< X3DNormalNode > normals_per_face;
			normals_per_face.reset( generateNormalsPerFace( x_dim, z_dim, 
				x_spacing, z_spacing, height, ccw ) );

			for( unsigned int face = 0; face < normals_per_face->nrAvailableNormals(); ++face ) {
...
X3DNormalNode *ElevationGrid::AutoNormal::generateNormalsPerVertex( 
	H3DInt32 x_dim, H3DInt32 z_dim, H3DFloat x_spacing, H3DFloat z_spacing,
	const vector< H3DFloat > &height, bool ccw, H3DFloat crease_angle ) {
		...
		if( x_dim > 1 && z_dim > 1 ) {
			...
			vector< Vec3f > normals( (x_dim -1) * (z_dim -1 ) * 4, Vec3f( 0, 0, 0 ) );
			normals.clear();
			...
			for( int face = 0; face < (int)normals_per_face->nrAvailableNormals(); ++face ) {
				...
				if( face / (quad_x_dim) != left_quad / ( quad_x_dim ) ) {
					left_quad = -1;
				}
				...
				if( face / (quad_x_dim) != right_quad / ( quad_x_dim ) ) {
					right_quad = -1;
				}

				int lower_quad = face + quad_x_dim;
				if( lower_quad >= quad_x_dim * quad_z_dim ) {
					lower_quad = -1;
				}
...
void ElevationGrid::SFBound::update() {
	...
	const vector< H3DFloat > &height = static_cast< MFFloat * >( routes_in[4] )->getValue();
