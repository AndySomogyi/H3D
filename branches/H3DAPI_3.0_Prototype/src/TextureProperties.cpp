...
//I am so sorry.
bool TextureProperties::operator==(const TextureProperties& rhs) {
	return (
		generateMipMaps->getValue()			== rhs.generateMipMaps->getValue() &&
		textureType->getValue()				== rhs.textureType->getValue() &&
		textureFormat->getValue()			== rhs.textureFormat->getValue() &&
		texturePriority->getValue()			== rhs.texturePriority->getValue() &&
		textureCompareFailValue->getValue() == rhs.textureCompareFailValue->getValue() &&
		anisotropicDegree->getValue()		== rhs.anisotropicDegree->getValue() &&
		borderColor->getValue()				== rhs.borderColor->getValue() &&
		borderWidth->getValue()				== rhs.borderWidth->getValue() &&
		boundaryModeR->getValue()			== rhs.boundaryModeR->getValue() &&
		boundaryModeS->getValue()			== rhs.boundaryModeS->getValue() &&
		boundaryModeT->getValue()			== rhs.boundaryModeT->getValue() &&
		magnificationFilter->getValue()		== rhs.magnificationFilter->getValue() &&
		minificationFilter->getValue()		== rhs.minificationFilter->getValue() &&
		textureCompareMode->getValue()		== rhs.textureCompareMode->getValue() &&
		textureCompression->getValue()		== rhs.textureCompression->getValue() &&
		textureTransferBias->getValue()		== rhs.textureTransferBias->getValue() &&
		textureTransferScale->getValue()	== rhs.textureTransferScale->getValue() 
	);
}
