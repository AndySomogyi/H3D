...
#define THREAD_LOCK_DEBUG_UPDATE_INTERVAL 2
	...
	if( di ) 
	{
		...
		for( DeviceInfo::MFDevice::const_iterator d = di->device->begin(); d != di->device->end(); ++d ) 
		{
	...
	else 
	{
		...
		TraverseInfo *ti = new TraverseInfo( vector< H3DHapticsDevice * >(), renderer.get());
		ti->setUserData( "ShadowCaster", shadow_caster.get() );
		ti->disableHaptics();
#ifdef HAVE_PROFILER
		H3DUtil::H3DTimer::stepBegin("Scene_traverse");
	...
	if (node)
	{
		...
		if (group)
		{
			...
			for( unsigned int i = 0; i < c.size(); ++i ) 
			{
				...
				if(n)
				{
					if (n->getTypeName() == nodeType)
					{
						if ((nodeName == "") || (nodeName == n->getName()))
						{
							return n;
						}
					}
		...
		if (shape)
		{
			...
			if (geom)
			{
				...
				{
					if((nodeName == "") || (nodeName == geom->getName()))
					{
						return geom;
					}
				}
			...
			if (appearance)
			{
				...
				{
					if((nodeName == "") || (nodeName == appearance->getName()))
					{
						return appearance;
					}
				}
		...
		if (appearance)
		{
			...
			if (material)
			{
				if (material->getTypeName() == nodeType)
				{
					...
					if((nodeName == "") || (nodeName == material->getName()))
					{	
						return material;
					}
				}
			...
			if( inlinenode->load->getValue() ) {
				for( unsigned int i = 0; i < inlinenode->loadedScene->size(); ++i ) 
				{
					...
					if( g )
					{
						...
						{
							if ((nodeName == "") || (nodeName == g->getName()))
							{
								return g;
							}
						}
