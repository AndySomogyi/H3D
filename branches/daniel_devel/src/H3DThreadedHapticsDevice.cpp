...
PeriodicThread::CallbackCode H3DThreadedHapticsDevice::forceEffectCallback( void *data ) {
  H3DThreadedHapticsDevice *hd = 
    ...
    if( has_intersection ) {
      ...
      new_proxy_pos = p + closest_intersection.normal * 1e-5;
    ...
    for( vector< Bounds::PlaneConstraint >::iterator i = constraints.begin();
      ...
      Vec3d vv = new_proxy_pos - hd->proxy_position;
      vv.normalizeSafe();
      if( (*i).lineIntersect( hd->proxy_position-vv*1e-7, 
...
//           cerr << new_proxy_pos << endl;
      ...
      new_proxy_pos = hd->proxy_position + (pos-hd->proxy_position)*0.05;
    ...
    if( intersected_constraints.size() > 1 ) {
