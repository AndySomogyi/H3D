  ...
  if( viscous_counter < 5 ) {
    ...
    if( ti.hapticsEnabled()  ) {
      if( enabled->getValue() ) {
        ...
        ti.addForceEffect( device_index, new HAPI::HapticViscosity( ti.getAccForwardMatrix(), viscosity->getValue() / 1000, radius->getValue() * 1000, dampingFactor->getValue(), false ) );
      }
    }
  }
}
